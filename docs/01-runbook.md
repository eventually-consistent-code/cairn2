...
| `NATIVE_MODULE_BROKEN` | The better-sqlite3 compiled binding is missing or built for a different node ABI — memory *index* tools (`mem_index`/`mem_search`/`mem_stats`/`mem_timeline`) fail; card tools keep working | Run the command in the message: `cd <server dir> && npm rebuild better-sqlite3`, then open `/plugin` and close it — plugin changes apply on menu close — or restart the session, so the server picks up the new binding. `config_probe` reports the same fix preemptively |
