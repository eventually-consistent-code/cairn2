...
function issuePoints(issue: Issue): number | null {
  if (issue.estimate?.points !== undefined) return issue.estimate.points;
...
): Promise<{ total: number | null; missing: number }> {
  if (!tracker || ids.length === 0) return { total: null, missing: ids.length };
  let total = 0;
  let found = 0;
  for (const id of ids) {
    try {
...
export async function estimatePhaseTokens(
  ...
  for (const [p, g] of [...byPhase.entries()].sort((a, b) => a[0] - b[0])) {
    ...
    if (info && info.issues.length > 0) {
      ...
      if (pts.total !== null && pts.total > 0) perPoint.push(g.tokens / pts.total);
    }
  }
