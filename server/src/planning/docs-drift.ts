...
export interface DocsDriftItem { phase: string; reason: string }
export interface DocsDriftReport { flagged: DocsDriftItem[]; ok: string[] }

// A verified phase, live or archived. `id` is the phase's path relative to
// the plans root minus the live "phases/" prefix -- "01-core" for a live
// phase, "milestones/v1/01-core" for an archived one -- so a report line
// maps straight back to a directory on disk.
interface VerifiedPhase {
  id: string; number: number; name: string; ledgerRel: string;
}

const DOC_EXT_RE = /\.(md|mdx|markdown)$/;

function verifiedPhases(projectDir: string): VerifiedPhase[] {
  const root = plansRoot(projectDir);
  const out: VerifiedPhase[] = [];
  for (const p of projectStatus(projectDir).phases) {
    if (!p.hasVerification) continue;
    out.push({
  ...
  if (existsSync(msDir)) {
    for (const v of readdirSync(msDir).filter((d) => /^v\d+$/.test(d))
      ...
      for (const entry of readdirSync(join(msDir, v))) {
        ...
        if (!parsed) continue;
        if (!existsSync(join(msDir, v, entry, "VERIFICATION.md"))) continue;
        out.push({
...
function gitTimestamp(projectDir: string, paths: string[]): number | undefined {
  try {
...
function docsCorpus(projectDir: string): string[] {
  const corpus: string[] = [];
  const changelog = join(projectDir, "CHANGELOG.md");
  if (existsSync(changelog)) corpus.push(normalizeForMatch(readFileSync(changelog, "utf8")));
  const walk = (dir: string): void => {
    for (const entry of readdirSync(dir)) {
      const path = join(dir, entry);
      if (statSync(path).isDirectory()) { walk(path); continue; }
      if (DOC_EXT_RE.test(entry)) corpus.push(normalizeForMatch(readFileSync(path, "utf8")));
    }
  };
 ...
 */
export function docsDriftReport(projectDir: string): DocsDriftReport {
  const flagged: DocsDriftItem[] = [];
  const ok: string[] = [];
