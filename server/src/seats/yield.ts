...
const YieldCountersSchema: z.ZodType<YieldCounters> = z.object({
  dispatched: CounterSchema,
  findingsRaised: CounterSchema,
  findingsSurvived: CounterSchema,
});
export const YieldStateSchema: z.ZodType<YieldState> = z.object({
  version: z.literal(1),
  project: z.string(),
...
export function yieldStatePath(
...
function freshState(projectDir: string): YieldState {
  return {
    version: 1,
...
function writeState(path: string, state: YieldState): void {
...
export function loadYield(
  projectDir: string, baseDir?: string,
): { state: YieldState; note?: string } {
...
export function recordYield(
  projectDir: string, deltas: YieldDelta[], baseDir?: string,
): { state: YieldState; note?: string } {
  for (const d of deltas) {
    if (!d.seat || !d.seat.trim()) {
      throw new CairnError("CONFIG_INVALID", "yield delta needs a seat name");
    }
    for (const field of ["dispatched", "findingsRaised", "findingsSurvived"] as const) {
      const v = d[field];
      if (v !== undefined && (!Number.isInteger(v) || v < 0)) {
        throw new CairnError(
          "CONFIG_INVALID",
          `yield delta for seat '${d.seat}': ${field} must be an integer >= 0, got ${v}`,
        );
      }
    }
  }
 ...
 */
export function yieldRate(c: YieldCounters): number | null {
  if (c.dispatched === 0) return null;
