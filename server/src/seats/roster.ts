  ...
  if (config?.enabled) {
    ...
    for (const name of config.enabled) {
      ...
      if (match?.valid) {
        ...
        notes.push(
          `enabled seat '${name}' matched no valid definition in templates/seats/ or .cairn/roles/ — skipped`,
        );
      }
    }
