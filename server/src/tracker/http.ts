 ...
 */
function rejectedNextAction(body: string): string {
...
async function fetchRaw(
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (resp.status === 400) {
      ...
      throw new CairnError("TRACKER_REJECTED", tag(withBody(`HTTP ${resp.status} from ${url}`, body)),
        rejectedNextAction(body));
    }
