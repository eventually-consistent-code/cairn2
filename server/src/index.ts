...
interface TimelineChunkItem {
  source: string;
  createdAt: string;
}
type TimelineItem = TimelineCardItem | TimelineChunkItem;

const timelineCmp = (a: string, b: string): number =>
  a < b ? -1 : a > b ? 1 : 0;
...
export function buildServer(deps: {
  ...
  const getDocsConnector = async (
    dOverride?: string,
  ): Promise<DocsConnector> => {
    const d = dOverride ?? dir();
  ...
  const refreshHandoff = (
    patch: Partial<Handoff> & { source: Handoff["source"] },
    d: string = dir(),
  ): void => {
    try {
  ...
  server.registerTool(
    ...
    wrap((a: { phase?: number | null; issueId?: string | null }) => {
      ...
      ctx.set(a);
      const state = ctx.get();
      const patch: Partial<Handoff> & { source: Handoff["source"] } = {
        source: "tool",
      };
  ...
  server.registerTool(
    ...
    {
      description:
        "Create an issue in the configured tracker. `phase` accepts the " +
    ...
    wrap(
      async (a: {
        ...
        const wantsEstimate =
          estimatePoints !== undefined || estimateMinutes !== undefined;
  ...
  server.registerTool(
    ...
    {
      description:
        "Update an issue (title/body/state/labels/assignee/estimate/phase). " +
    ...
    wrap(
      async (a: {
        ...
        const phase =
          phaseParam === undefined || !canRephase
            ? undefined
        ...
        if (patch.state === "in_progress" && patch.assignee === undefined) {
          ...
          try {
            ...
            if (!current.assignee) {
              const who =
                loadConfig(d).user?.handle ?? (await tracker.resolveSelf?.());
              if (who) {
                patch.assignee = who;
                autoAssigned = true;
              }
  ...
  const linkCapable = async (): Promise<Tracker> => {
    ...
    if (!t.capabilities.hasDependencies || !t.linkIssues) {
      throw new CairnError(
        "UNSUPPORTED",
        "this tracker has no dependency links",
  ...
  server.registerTool(
    ...
    {
      description:
        "Link two issues (blocks/parent-of/relates-to/supersedes); " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Remove an issue link; UNSUPPORTED unless the tracker hasDependencies",
      inputSchema: z.object({
        from: z.string(),
        type: LinkTypeEnum,
        to: z.string(),
  ...
  server.registerTool(
    ...
    {
      description:
        "List issue links — for one issue (either direction) or the whole project",
  ...
  server.registerTool(
    ...
    {
      description:
        "Dependency-graph report: ready frontier (open issues with no open " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Promote a local-tracker project to a hosted backend: phases, " +
    ...
    wrap(
      async (a: {
        ...
        const storeDir = resolve(
          d,
          (cfg.tracker.config as { dir?: string }).dir ?? ".tracker",
        );
  ...
  server.registerTool(
    ...
    {
      description:
        "List issues, optionally by phase/state (phase accepts the tracker's " +
    ...
    wrap(async (a: { phase?: string; state?: IssueState }) => {
  ...
  server.registerTool(
    ...
    wrap(async (a: { name: string }) =>
      (await getTracker()).createPhase(a.name),
    ),
  );

  server.registerTool(
    "phase_list",
    { description: "List phases", inputSchema: z.object({}) },
  ...
  server.registerTool(
    ...
    {
      description:
        "Create .cairn/plans/PROJECT.md + roadmap.md (never overwrites)",
      inputSchema: z.object({ name: z.string() }),
    },
  ...
  server.registerTool(
    ...
    {
      description:
        "Create phases/NN-slug/ (or NN.F-slug/ for a decimal insert) with " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Ensure the tracker has a phase named 'Phase N: <name>' (idempotent)",
  ...
  server.registerTool(
    ...
    {
      description:
        "Index reference material into the searchable memory store (disposable, rebuildable)",
      inputSchema: z.object({
        content: z.string(),
        source: z.string(),
  ...
  server.registerTool(
    ...
    {
      description:
        "Full-text search the memory index, optionally scoped to a phase/issue",
      inputSchema: z.object({
        query: z.string(),
  ...
  server.registerTool(
    ...
    {
      description:
        "Write a durable memory card (decision/constraint/gotcha/reference/note) with provenance",
      inputSchema: z.object({
  ...
  server.registerTool(
    ...
    {
      description:
        "List memory cards with staleness checked against their provenance (the anti-rot check)",
      inputSchema: z.object({
        scopePhase: z.number().optional(),
        scopeIssue: z.string().optional(),
        scopeRole: z.string().optional(),
  ...
  server.registerTool(
    ...
    {
      description:
        "Retro-gated card compaction (#172) -- the capacity guard's action. 'propose' reports " +
    ...
    wrap((a: { mode: "propose" | "apply"; ids?: string[]; threshold?: number }) => {
      const d = dir();
      if (a.mode === "propose") return proposeCompaction(d, { threshold: a.threshold });
      if (a.ids === undefined || a.ids.length === 0) {
        throw new CairnError("CONFIG_INVALID", "mode 'apply' needs the approved ids",
          "run mode 'propose' first and pass the ids it lists under 'retiring'");
      }
  ...
  server.registerTool(
    ...
    {
      description:
        "Chronological neighbors around an anchor (a memory card id or an index chunk source) -- " +
    ...
    wrap((a: { anchor: string; before?: number; after?: number }) => {
      ...
      try {
        ...
        if (chunkCreatedAt === undefined) {
          throw new CairnError(
            "NOT_FOUND",
            `no card or index chunk '${a.anchor}'`,
      ...
      const cardsBefore = cardItems.filter((c) =>
        isBeforeAnchor(c.created, c.id),
      );
      const cardsAfter = cardItems.filter((c) =>
        isAfterAnchor(c.created, c.id),
      );

      const chunkNeighbors: SearchResult[] = getMemIndex(d).timeline(
        anchorCreatedAt,
        before,
        after,
      );
  ...
  server.registerTool(
    ...
    {
      description:
        "Tracker issues (non-closed) that no phase's PLAN.md references — work at risk of being missed",
      inputSchema: z.object({}),
    },
    wrap(async () => {
      const d = dir();
  ...
  server.registerTool(
    ...
    {
      description:
        "Reverse-mirror a tracker phase (by id or name substring) into .cairn/plans/ artifacts",
  ...
  server.registerTool(
    ...
    {
      description:
        "Write/refresh the session handoff (checkpoint) for this project",
      inputSchema: z.object({
  ...
  server.registerTool(
    ...
    {
      description:
        "Read the current session handoff, if any (flags handoffs older than 14 days as stale, never errors on staleness)",
      inputSchema: z.object({}),
    },
  ...
  server.registerTool(
    ...
    {
      description:
        "Start the next milestone — native tracker object when the backend supports it; stamps milestone_id into roadmap.md",
      inputSchema: z.object({ name: z.string() }),
    },
    wrap(async (a: { name: string }) => {
      const d = dir();
  ...
  server.registerTool(
    ...
    {
      description:
        "Current milestone number, archived milestones, and the tracker's native list when supported",
      inputSchema: z.object({}),
    },
    wrap(async () => {
      const d = dir();
  ...
  server.registerTool(
    ...
    {
      description:
        "Complete the current milestone: gate on all-phases-verified, close tracker phases, " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Detect out-of-band commits (covered by no LEDGER.md range) since the last resync marker; " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Diff the live tracker against the last-seen snapshot cursor: new issues/phases, " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Set wave grouping (wave_N frontmatter) and/or the TDD-eligible task list on a phase's PLAN.md",
      inputSchema: z.object({
        phaseDir: z.string(),
  ...
  server.registerTool(
    ...
    {
      description:
        "Read cairn.json as the validated, post-defaults effective config",
      inputSchema: z.object({}),
    },
    wrap(() => loadConfig(dir())),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Merge-patch cairn.json (null deletes a key). Validates the merged result before " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Credential preflight (CRN-48) -- one cheap authenticated call to the configured " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Post a plain-language comment on a tracker issue (management-visible progress note)",
  ...
  server.registerTool(
    ...
    {
      description:
        "Attach a file (screenshot, render, visual evidence) to a tracker issue. " +
    ...
    wrap(async (a: { id: string; path: string; filename?: string }) => {
      ...
      if (!tracker.capabilities.hasIssueAttachments || !tracker.attachFile) {
        throw new CairnError(
          "UNSUPPORTED",
          "this tracker has no attachment support",
  ...
  server.registerTool(
    ...
    {
      description:
        "Open a persistent debugging session (.cairn/trace/<id>.md). Creates the tracker " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Append a typed entry (evidence|hypothesis|test|verdict) to an open trace — append-only",
      inputSchema: z.object({
        id: z.string(),
    ...
    wrap(
      (a: {
        id: string;
        kind: "evidence" | "hypothesis" | "test" | "verdict";
        text: string;
      }) => {
        const d = dir();
  ...
  server.registerTool(
    ...
    {
      description:
        "List trace sessions (open and/or resolved) with entry counts",
      inputSchema: z.object({
  ...
  server.registerTool(
    ...
    {
      description:
        "Resolve a trace: requires >=1 evidence entry, >=1 test entry (the first test is the repro: " +
    ...
    wrap(async (a: { id: string; resolution: string }) => {
      ...
      if (out.issue) {
        ...
        try {
          ...
          await tracker.commentIssue(out.issue,
  ...
  const registerSessionTools = (
    ...
    server.registerTool(
      ...
      {
        description:
          `Open a persistent ${kind} session (.cairn/${kind}/<id>.md). Creates the tracker ` +
      ...
      wrap(async (a: { description: string; issueId?: string }) => {
        ...
        const phase =
          active.phase !== undefined ? String(active.phase) : undefined;
    ...
    server.registerTool(
      ...
      {
        description:
          `Resolve a ${kind} session: requires a ${spec.closeGate} entry; archives it, comments ` +
  ...
  server.registerTool(
    ...
    {
      description:
        "Deterministic join over trace/probe/draft/thread sessions — open + resolved with " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Merge-patch the project knowledge graph (.cairn/map/map.json) -- nodes merge by id " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Read the project knowledge graph, optionally filtered by nodeType, edgeType, or a " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Composite graph query: node + depth (0-3, default 1) walks a BFS neighborhood over " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Workspace name, root, members (name/path/configured), and current focus. " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Set (or clear, with project: null) the workspace focus — every tool then " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Curated cross-project read: per configured member { name, phase, openIssues, " +
    ...
    wrap(async () => {
      ...
      for (const m of info.members.filter((member) => member.configured)) {
        try {
          ...
          if (!t) {
            t = new CachedTracker(
              await makeTracker(loadConfig(m.absPath), m.absPath),
            );
          ...
          const openSessions = Object.values(openByKind).reduce(
            (sum, n) => sum + n,
            0,
          );
  ...
  server.registerTool(
    ...
    {
      description:
        "Read the workspace dispatch board (.cairn/basecamp/board.json) — workstreams " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Merge-patch the dispatch board: workstreams merge by id (null deletes); " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Re-derive another registered project's outlook snapshot in place (#92) -- " +
    ...
    wrap((a: { project: string }) => {
      const entries = readRegistry().projects.filter(
        (p) =>
          p.path === a.project ||
      ...
      if (entries.length === 0) {
        throw new CairnError(
          "NOT_FOUND",
          `no registered project matches '${a.project}'`,
  ...
  server.registerTool(
    ...
    {
      description:
        "Prune a project from the machine registry (#92) -- outlook forget <project>. " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Explicit outlook snapshot emit for lifecycle gates (verify/ship/summit). " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Portfolio aggregate (#55): every registered cairn project on this machine as a " +
    ...
    wrap((a: { artifact?: boolean }) =>
      outlookAggregate(undefined, a.artifact ? { artifact: true } : undefined),
    ),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "Detected external AI peer CLIs (codex/opencode/antigravity/grok) — {peers: [on PATH, enabled, " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Run one external peer CLI with capped stdin input — advisory output, non-zero exit is a result. " +
  ...
  server.registerTool(
    ...
    {
      description:
        "Per-run peers convergence state (.cairn/peers/<slug>/) — ops: start (mode/target/" +
    ...
    wrap(
      (a: {
        ...
        switch (a.op) {
          ...
          case "record_output":
            return recordPeerOutput(
              d,
              a.slug,
              need(a.peer, "peer"),
              need(a.round, "round"),
              need(a.output, "output"),
            );
          case "record_findings":
            ...
            return recordFindings(
              d,
              a.slug,
              need(a.peer, "peer"),
              need(a.round, "round"),
              need(a.findings, "findings") as unknown as Finding[],
            );
          case "verdict":
            return recordVerdict(
              d,
              a.slug,
              need(a.findingId, "findingId"),
              need(a.verdict, "verdict"),
              a.note,
            );
          case "status":
  ...
  server.registerTool(
    ...
    {
      description:
        "Parse a research artifact's ##+ section markers " +
    ...
    wrap(
      (a: {
        ...
        if (realRel === "" || realRel.startsWith("..") || isAbsolute(realRel)) {
          throw new CairnError(
            "CONFIG_INVALID",
            `path '${a.path}' resolves outside the project directory (symlink escape)`,
            "research artifacts must really live under the project root",
          );
        }
        let markdown: string;
        try {
  ...
  server.registerTool(
    ...
    {
      description:
        "Docs connector status — configured connector and the project's landing page, when one exists. " +
    ...
    wrap(async (a: { projectName?: string }) => {
      ...
      try {
        const connector = await getDocsConnector(d);
        const root = await connector.findPage(
          a.projectName ?? defaultProjectName(d),
        );
  ...
  server.registerTool(
    ...
    {
      description:
        "Assemble one phase's distill manifest — the scope an incremental distill run may " +
