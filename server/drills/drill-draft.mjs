...
const started = await call("draft_start", {
  description: "drill: dashboard density — cards or table?",
});
check(
  "draft_start: session + real cairn:sketch issue",
  Boolean(started.id) && Boolean(started.issue),
  JSON.stringify(started.__error ?? "").slice(0, 100),
);
check(
  "sketch issue: label cairn:sketch",
  (await call("issue_get", { id: started.issue })).labels?.includes(
    "cairn:sketch",
  ),
);
...
check(
  "theme: custom properties only — no selectors beyond :root, no layout",
  /^:root \{/.test(theme) &&
    !/display:|grid-template|margin:|\.[a-z]/.test(theme),
);
...
writeFileSync(
  join(variantDir, "001-cards.html"),
  page("Cards", '<main class="cards">card grid variant</main>'),
);
writeFileSync(
  join(variantDir, "002-table.html"),
  page("Table", '<main class="table">flat table variant</main>'),
);

for (const f of ["001-cards.html", "002-table.html"]) {
  const html = readFileSync(join(variantDir, f), "utf8");
  check(
    `variant ${f}: links ../themes/default.css`,
    html.includes('href="../themes/default.css"'),
  );
}
...
const RESOLUTION =
  "card grid direction locked; table reserved for export view.";
const closed = await call("draft_close", {
  id: started.id,
  resolution: RESOLUTION,
});
check(
  "draft_close: resolved, issue closed",
...
writeFileSync(
  join(skillDir, "references", "css-patterns.md"),
  [
    "# CSS Patterns",
    "",
    "## Design Decisions",
    ...
    "```",
    "## What to Avoid",
    "- component styles or layout rules in the theme file — custom properties only",
    "## Origin",
    `- session: ${started.id}`,
...
check(
  "wrap: CSS-pattern synthesis with provenance",
  ref.includes("--color-primary") &&
    ref.includes(started.id) &&
    ref.includes(started.issue),
);
