...
check(
  "drift: finding on the consumer, counterpart names the producer",
  drift?.plan?.includes("02-dashboard") &&
    drift?.counterpart?.plan?.includes("01-export"),
);
check("drift: consumer line correct (line 3)", drift?.line === 3);
check(
  "threshold: flagged with the matched text (line 5)",
...
check(
  "two calls byte-equal on unchanged tree",
