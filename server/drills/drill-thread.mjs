...
check(
  "thread_start: session + real cairn:thread issue",
  Boolean(started.id) && Boolean(started.issue),
  JSON.stringify(started.__error ?? "").slice(0, 100),
);
check(
  "issue: label cairn:thread, open",
  (await call(c1, "issue_get", { id: started.issue })).labels?.includes(
    "cairn:thread",
  ),
);

const mirror1 =
  "Thread opened: migrating the payments adapter. Long-running context lives here so nothing gets re-derived between sessions.";
await call(c1, "issue_comment", { id: started.issue, text: mirror1 });

await call(c1, "thread_log", {
  id: started.id,
  kind: "note",
  text: "stripe adapter first; paypal after the interface settles",
});
await call(c1, "thread_log", {
  id: started.id,
  kind: "link",
  text: "probe-deadbeef — proved streaming exports hold at 1M rows, which unblocks the reconciliation piece",
});
await call(c1, "thread_log", {
  id: started.id,
  kind: "decision",
  text: "webhooks over polling — polling burned the rate limit in the probe",
});

check(
  "banner: open thread surfaces last",
...
check(
  "resume: already-open guard fires (resume IS the point)",
...
check(
  "resume: counts intact, zero re-derived context",
  resumed?.status === "open" &&
    resumed.entryCounts?.note === 1 &&
    resumed.entryCounts?.link === 1 &&
    resumed.entryCounts?.decision === 1,
);

// -- wrap gate ------------------------------------------------------------------
const early = await call(c2, "thread_close", {
  id: started.id,
  resolution: "done-ish",
});
...
const mirror2 =
  "Thread wrapped: stripe migrated on webhooks; paypal queued behind the settled interface. Decisions and links live in the thread archive.";
await call(c2, "issue_comment", { id: started.issue, text: mirror2 });
await call(c2, "thread_log", {
  id: started.id,
  kind: "wrap",
  text: "landed: stripe on webhooks, interface settled, paypal is the next thread",
});
const RESOLUTION =
  "Payments thread wrapped — stripe migrated, paypal queued as its own thread.";
const closed = await call(c2, "thread_close", {
  id: started.id,
  resolution: RESOLUTION,
});
check(
  "thread_close: wrapped + issue closed",
...
function resolvedArchive() {
  try {
    readFileSync(
      join(PROJECT, ".cairn", "thread", "archive", `${started.id}.md`),
      "utf8",
    );
    return true;
  } catch {
...
check(
  "story: opened → wrapped → resolved, in order",
  comments.length === 3 &&
    comments[0] === mirror1 &&
    comments[1] === mirror2 &&
