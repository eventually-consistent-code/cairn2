...
check("mechanical finding: real cairn:audit issue", mech.labels?.includes("cairn:audit"),
  JSON.stringify(mech.__error ?? "").slice(0, 100));
check("investigation finding: real cairn:audit issue", invest.labels?.includes("cairn:audit"));
check("severity is the body's first line",
  mech.body?.split("\n")[0] === "important" && invest.body?.split("\n")[0] === "critical");

// -- record written, findings carry issue ids -----------------------------------
const rec1 = await call("audit_record", {
  scope: "uat-phase-1", verdict: "findings",
  findings: [
    { severity: "important", title: "settings page shows old plan-name copy", issue: mech.id },
    { severity: "critical", title: "checkout total drifts on multi-currency carts", issue: invest.id },
    { severity: "minor", title: "empty-state illustration misaligned at narrow widths" },
  ],
});
check("record written with three findings", rec1.findings === 3,
  JSON.stringify(rec1.__error ?? "").slice(0, 100));
const recRaw = readFileSync(join(PROJECT, ".cairn", "audit", `uat-phase-1-${today}.md`), "utf8");
check("record: minor stays record-only, issues linked",
...
await call("issue_comment", { id: mech.id,
...
check("mechanical finding closed", (await call("issue_get", { id: mech.id })).state === "closed");
...
check("trace opened on the finding's issue — no inline fix", Boolean(trace.id) && trace.issue === invest.id);
await call("trace_log", { id: trace.id, kind: "evidence",
...
check("trace visible in the landscape", landscape.sessions?.some((s) => s.id === trace.id && s.status === "open"));

// -- same-day supersede -----------------------------------------------------------
const rec2 = await call("audit_record", {
  scope: "uat-phase-1", verdict: "findings",
  findings: [{ severity: "critical", title: "checkout total drifts on multi-currency carts", issue: invest.id }],
});
const recRaw2 = readFileSync(rec2.path, "utf8");
check("same-day re-run supersedes the record",
  rec2.findings === 1 && !recRaw2.includes("plan-name copy"));

// -- leak scan over everything sent to the tracker ---------------------------------
const sent = [mech.body, invest.body,
...
await call("trace_close", { id: trace.id,
  resolution: "Rounding now happens once at the total, so cart and confirmation agree to the cent." });
