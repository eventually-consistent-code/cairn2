...
check("new step 1: cairn.json exists gate", existsSync(join(PROJECT, "cairn.json")));
const proj = await call("plan_scaffold_project", { name: "dogfood" });
check("new: project scaffolded", !proj.__error, JSON.stringify(proj.__error ?? "").slice(0, 80));
const ph = await call("plan_scaffold_phase", { number: 1, name: "first ascent" });
const phaseDir = ph.phaseDir ?? "01-first-ascent";
await call("plan_phase_ensure", { number: 1, name: "first ascent" });
const req = await call("issue_create", { title: "greet the user from the readme",
  body: "README should say hello like it means it." });
await call("plan_issues_set", { phaseDir, issues: [req.id] });
check("new: requirement issued + linked to the phase plan",
...
check("plan: phase 1 visible with its issue", Boolean(phase1) && phase1.hasPlan !== undefined);
...
await call("continuity_checkpoint", {
  issue: req.id,
  task: { current: "greet-from-readme", title: "greet the user from the readme" },
  next_action: "write the greeting + commit",
});

// the work itself
writeFileSync(join(PROJECT, "README.md"), "# dogfood scratch\n\nWell hello there — glad you made it.\n");
git("add", "-A"); git("commit", "-q", "-m", "feat: readme greets like it means it");
...
execFileSync("node", [join(REPO, "hooks", "scripts", "posttooluse-breadcrumb.mjs")], {
  env: { ...process.env, CLAUDE_PROJECT_DIR: PROJECT }, input: "{}", cwd: PROJECT });

// fresh session: SessionStart hook injects the resume block
const resume = execFileSync("node", [join(REPO, "hooks", "scripts", "sessionstart-continuity.mjs")], {
  env: { ...process.env, CLAUDE_PROJECT_DIR: PROJECT }, input: "{}", cwd: PROJECT, encoding: "utf8" });
check("resume: SessionStart hook names the exact task",
...
check("resume: continuity_get lands on the exact task",
  (cont.task?.current ?? cont.handoff?.task?.current) === "greet-from-readme");
check("resume: tracker cross-check — issue still in_progress, no contradiction",
...
await call("ledger_append", { phaseDir, taskRef: "greet-from-readme",
  summary: "readme greets like it means it", baseCommit: base, headCommit: head,
  issueId: req.id, closedDate: new Date().toISOString().slice(0, 10) });
check("work: issue closed + ledgered with real shas",
  readFileSync(join(PROJECT, ".cairn", "plans", "phases", phaseDir, "LEDGER.md"), "utf8")
    .includes(head.slice(0, 7)));

// -- /cairn verify 1 (verify.md steps 2,3,5) ------------------------------------
console.log("/cairn verify 1 ...");
const drift = await call("plan_drift", {});
check("verify: drift correctly flags the closed issue while the phase is unverified",
  !drift.__error && JSON.stringify(drift).includes(req.id));
const openLeft = await call("issue_list", { state: "open" });
check("verify: no open issues left on the phase",
  Array.isArray(openLeft) && !openLeft.some((i) => i.id === req.id));
writeFileSync(join(PROJECT, ".cairn", "plans", "phases", phaseDir, "VERIFICATION.md"),
...
const verified = shipStatus.phases?.find((p) => p.number === 1)?.hasVerification;
check("ship gate: drift clean + phase verified", !drift2.__error && verified === true);
await call("continuity_clear", {});
const afterClear = await call("continuity_get", {});
check("ship: continuity cleared (session ends shipped)",
  afterClear === null || afterClear?.handoff === null || Boolean(afterClear?.__error) || !afterClear?.task);

// -- /cairn do "what's the status" — routes to status without confirmation ------
console.log('/cairn do "what\'s the status" ...');
const skill = readFileSync(join(REPO, "skills", "cairn-trailhead", "SKILL.md"), "utf8");
check("do-routing: status is a live routed verb the table resolves",
  /\|\s*`status`\s*\|.*\|\s*live\s*\|/.test(skill));
check("do-routing: status renders from live reads", !(await call("plan_status", {})).__error);

// -- /cairn wrok — nearest-match per help.md -------------------------------------
const verbs = [...skill.matchAll(/\|\s*`([a-z-]+)`\s*\|/g)].map((m) => m[1]);
const dist1 = (a, b) => { // adjacent transposition or 1 substitution catches wrok→work
...
check('wrok: not a verb; nearest-match table resolves to exactly "work"',
  !verbs.includes("wrok") && guesses.length === 1 && guesses[0] === "work");
check("help.md states the rule verbatim",
  readFileSync(join(REPO, "skills", "cairn-trailhead", "verbs", "help.md"), "utf8").includes("did you mean `work`?"));
