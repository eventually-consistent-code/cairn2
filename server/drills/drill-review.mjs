...
const slug = TARGET.toLowerCase()
  ...
  .replace(/^-|-$/g, "");
check(
  "slug rule survives audit_record's scope gate",
  slug === "feature-export-42",
);
  ...
  findings: [
    {
      ...
      detail:
        "src/export/handler.ts:87 — catch block logs and falls through to res.ok; failure scenario: disk full mid-write returns 200 with a truncated file.",
      issue: issue.id,
    },
    {
      severity: "minor",
      title: "exporter test names drifted from behavior (clarity axis)",
    },
  ],
});
check(
  "record under the slugged scope",
  rec.path?.endsWith(`review-${slug}-${today}.md`),
  JSON.stringify(rec.__error ?? "").slice(0, 100),
);
const raw = readFileSync(rec.path, "utf8");
check(
  "record carries file:line + failure scenario; minor record-only",
  raw.includes("handler.ts:87") && raw.includes("## finding — minor"),
);
check(
  "issue body carries NO file:line (audience split held)",
  !/:\d+/.test(issue.body ?? "x:1") || !issue.body.includes("handler.ts"),
);
