...
const HOOK = join(dirname(fileURLToPath(import.meta.url)), "..", "..", "hooks", "scripts", "pretooluse-leakguard.mjs");
...
const hook = (command) => spawnSync("node", [HOOK], {
  cwd: PROJECT,
...
check("listing has path:line + both patterns",
  /app\.ts:1: \[cairn-path\]/.test(r.stderr) && /app\.ts:1: \[tracker-id\] DRILL-42/.test(r.stderr),
  r.stderr.split("\n")[1] ?? "");

// -- case 2: fixed file passes ----------------------------------------------
writeFileSync(join(PROJECT, "app.ts"), "const planPath = process.env.PLAN_PATH;\n");
git("add", "app.ts");
...
writeFileSync(join(PROJECT, "app.ts"), 'const leak = ".cairn/handoff"; // DRILL-9\n'); // tracked, NOT staged
r = hook("git commit -am 'sweep everything'");
check("-am widened scan catches unstaged tracked leak → exit 2", r.status === 2);
...
const cfg = await call("config_set", { patch: { leakGuard: { enabled: false } } });
check("config_set flips leakGuard.enabled false", cfg.leakGuard?.enabled === false);
