...
const reqTraced = await call("issue_create", {
  title: "dashboard shows live export progress", labels: ["cairn:backlog"],
  body: "Users need to see export progress without refreshing." });
const reqUntraced = await call("issue_create", {
  title: "settings page gets a dark mode toggle", labels: ["cairn:backlog"],
...
const session = await call("draft_start", { description: "drill: export progress surface" });
...
writeFileSync(join(themesDir, "tokens.json"), JSON.stringify({
  color: { primary: "#2563eb", bg: "#fafafa" }, space: { "2": "0.5rem" },
}, null, 2) + "\n");
...
const tokenProps = Object.entries(tokens)
  .flatMap(([g, o]) => Object.keys(o).map((k) => `${g}-${k}`)).sort();
check("tokens.json ↔ default.css in sync", JSON.stringify(cssProps) === JSON.stringify(tokenProps),
  `${cssProps} vs ${tokenProps}`);

await call("draft_log", { id: session.id, kind: "variant",
  text: "001-progress-bar.html — inline bar under the export button" });
const DECISION = "inline progress bar under the export button; polling every 2s per the probe's rate findings";
...
check("both traceability edges recorded", withEdges.edges.length === 2
...
for (const req of [reqTraced, reqUntraced]) {
  const hasImplements = scape.edges.some((e) => e.from === req.id && e.type === "implements");
  if (!hasImplements) gaps.push(req);
}
check("sweep names exactly the seeded gap", gaps.length === 1 && gaps[0].id === reqUntraced.id);
const rec = await call("audit_record", { scope: "uat-frontend-drill", verdict: "findings",
  findings: [{ severity: "important",
    title: "requirement has no decided design — dark mode toggle untraced",
    detail: "no implements edge from the requirement issue; the designer never saw it",
...
const FIDELITY_BODY = "important\nShipped dashboard uses a modal progress dialog — the draft session decided an INLINE bar under the export button. Violates the decision entry: \"" + DECISION + "\"";
const fidelity = await call("issue_create", {
  title: "shipped progress UI diverges from the decided direction",
  labels: ["cairn:audit"], body: FIDELITY_BODY });
check("fidelity finding cites the decision entry",
  fidelity.labels?.includes("cairn:audit") && fidelity.body.includes("INLINE bar"));

// -- drift detection: seed tokens/CSS divergence ------------------------------------
console.log("drift: seeded token divergence detected...");
writeFileSync(join(themesDir, "tokens.json"), JSON.stringify({
  color: { primary: "#2563eb", bg: "#fafafa", danger: "#ef4444" }, space: { "2": "0.5rem" },
}, null, 2) + "\n");
const tokens2 = JSON.parse(readFileSync(join(themesDir, "tokens.json"), "utf8"));
const tokenProps2 = Object.entries(tokens2)
  .flatMap(([g, o]) => Object.keys(o).map((k) => `${g}-${k}`)).sort();
check("drift detected (token without a CSS twin)",
  JSON.stringify(cssProps) !== JSON.stringify(tokenProps2)
  && tokenProps2.includes("color-danger") && !cssProps.includes("color-danger"));

// -- close out + leak scan -----------------------------------------------------------
await call("draft_close", { id: session.id, resolution: "inline progress bar locked; prototype in the session artifacts" });
await call("issue_comment", { id: fidelity.id, text: "Fixed: dashboard back to the inline bar per the decided direction; modal removed." });
await call("issue_close", { id: fidelity.id });
for (const r of [reqTraced, reqUntraced]) await call("issue_close", { id: r.id });
