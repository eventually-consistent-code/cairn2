...
// -- byte-equal reads --------------------------------------------------------------
check("two reads byte-equal", JSON.stringify(await call("map_get", {})) === JSON.stringify(before));

// -- filters -----------------------------------------------------------------------
const nodeView = await call("map_get", { node: "mod-export" });
check("node filter: self + touching edges + neighbors",
  Boolean(nodeView.nodes["mod-export"]) && Boolean(nodeView.nodes["mod-dashboard"])
  && Boolean(nodeView.nodes["dec-streaming"]) && nodeView.edges.length === 2);
const mods = await call("map_get", { nodeType: "module" });
check("nodeType filter narrows to modules", Object.keys(mods.nodes).length === 2);
const decideEdges = await call("map_get", { edgeType: "decided-in" });
check("edgeType filter narrows edges", decideEdges.edges.length === 1
  && decideEdges.edges[0].to === "dec-streaming");
...
check("null-delete works once detached", deleted.nodes === 2 && deleted.edges === 1);
