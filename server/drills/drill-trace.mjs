...
check(
  "trace_start: session + real bug issue",
...
check(
  "bug issue: label cairn:bug, open",
  issue.labels?.includes("cairn:bug") && issue.state === "open",
);

const mirror1 =
  "Investigation started: retries are firing twice when the tracker returns a 502. Reproducing before any theory.";
const posted1 = await call(c1, "issue_comment", {
  id: started.issue,
  text: mirror1,
});
check("mirror comment #1: investigation started", !posted1.__error);

await call(c1, "trace_log", {
  id: started.id,
  kind: "evidence",
...
check(
  "mid-session: open with 1/1/1/0 entry counts",
  mid?.entryCounts?.evidence === 1 &&
    mid?.entryCounts?.hypothesis === 1 &&
    mid?.entryCounts?.test === 1 &&
    mid?.entryCounts?.verdict === 0,
);

const banner = renderBanner(PROJECT);
check(
  "banner: open trace surfaces",
  typeof banner === "string" && banner.includes(started.id),
);
...
check(
  "resume: trace intact, zero re-derived evidence",
  resumed?.entryCounts?.evidence === 1 &&
    resumed?.entryCounts?.hypothesis === 1 &&
...
const lastHeader = [
  ...body.matchAll(/^## (evidence|hypothesis|test|verdict) — /gm),
].at(-1)?.[1];
check("resume: last entry is exactly where we left off", lastHeader === "test");

// -- verdict gate: close refuses without a verdict --------------------------
const premature = await call(c2, "trace_close", {
  id: started.id,
  resolution: "nope",
});
check(
  "verdict gate: close without verdict refused",
  premature.__error?.code === "PRECONDITION_FAILED" ||
    Boolean(premature.__error),
);

// -- cause found, verdict, close --------------------------------------------
const mirror2 =
  "Cause identified: two layers both owned retry — the request wrapper and the queue worker each retried the same failed call.";
await call(c2, "issue_comment", { id: started.issue, text: mirror2 });

await call(c2, "trace_log", {
  id: started.id,
  kind: "verdict",
...
const RESOLUTION =
  "Retries now happen in one place (the queue). The wrapper fails fast, so a 502 produces exactly one retry.";
const closed = await call(c2, "trace_close", {
  id: started.id,
  resolution: RESOLUTION,
});
check(
  "trace_close: resolved + tracker issue closed",
  closed.issueClosed === true && closed.verdicts?.length === 1,
);

const after = await call(c2, "issue_get", { id: started.issue });
check("bug issue: closed on tracker", after.state === "closed");
check(
  "archive: session file moved, live file gone",
  !existsSync(sessionFile) &&
    existsSync(join(PROJECT, ".cairn", "trace", "archive", `${started.id}.md`)),
);
...
const card = await call(c2, "mem_card_create", {
  ...
  provenanceFiles: ["src/queue/worker.ts"],
  provenanceCommits: ["abc1234"],
});
check(
  "gotcha card: created with provenance",
  card.id?.startsWith("gotcha-") === true,
);
const recalled = await call(c2, "mem_card_recall", {});
check(
  "gotcha card: recallable in a later session",
  Array.isArray(recalled) &&
    recalled.some((c) => (c.card?.id ?? c.id) === card.id),
);
...
check(
  "story: three comments in order (started → cause → resolved)",
  comments.length === 3 &&
    comments[0] === mirror1 &&
    comments[1] === mirror2 &&
...
check(
  "leak scan: zero pattern hits across all comment text",
  hits.length === 0,
