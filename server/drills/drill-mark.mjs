...
check("backlog: one call → real issue", Boolean(backlog.id), JSON.stringify(backlog.__error ?? "").slice(0, 100));
check("backlog: bare — label only, no body", backlog.labels?.includes("cairn:backlog") && (backlog.body ?? "") === "");
...
check("seed: one call → real issue with trigger body",
  Boolean(seed.id) && seed.labels?.includes("cairn:seed") && /^Trigger: /.test(seed.body ?? ""));
...
check("note: recallable via mem_card_list (scoped)",
  Array.isArray(listed) && listed.some((c) => c.id === note.id));
const recalled = await call("mem_card_recall", { scopePhase: 1 });
check("note: mem_card_recall returns it",
  Array.isArray(recalled) && recalled.some((c) => (c.card?.id ?? c.id) === note.id));

// tracker read-back: both marks visible, bare
const back = await call("issue_get", { id: backlog.id });
check("tracker read-back: backlog mark intact", back.labels?.includes("cairn:backlog"));
