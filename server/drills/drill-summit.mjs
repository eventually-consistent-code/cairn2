...
// ---------------------------------------------------------------
console.log(
  `backend: ${cfg.tracker.type} (${NATIVE ? "native milestones" : "fallback"})`,
);
...
for (const [num, dir, tp] of [
  ...
  execFileSync(
    ...
    [
      "-c",
      `printf '# Phase ${num} — verified\\n' > .cairn/plans/phases/${dir}/VERIFICATION.md`,
    ],
    { cwd: PROJECT },
  );
  git("add", "-A");
  git("commit", "-m", `docs: verify phase ${num}`, "--no-gpg-sign");
}
...
check(
  "milestone_list current=1, nothing stamped",
  ml.current === 1 && !ml.currentId,
);
...
if (NATIVE) {
  ...
  console.log(
    "fallback: no bootstrap offer (hasMilestones false) — correct per summit.md step 1",
  );
}
...
const report = await call("milestone_complete", {
  summary: "summit drill: both phases shipped",
});
check(
  "no tool error",
  !report.__error,
  JSON.stringify(report.__error ?? "").slice(0, 120),
);
check("2 tracker phases closed", report.closedPhases?.length === 2);
check(
  "no skipped phase closes",
  (report.skippedPhases?.length ?? 0) === 0,
...
if (NATIVE) {
  ...
  check(
    "native release skipped (recorded, not errored)",
    report.released === undefined,
  );
}
...
check(
  "phases/ archived",
  !existsSync(join(PROJECT, ".cairn/plans/phases", p1.dir)) &&
...
check(
  "roadmap milestone: 2 + archive note",
  /milestone: 2/.test(roadmap) &&
...
git("add", "-A");
git("commit", "-m", "chore(cairn): summit — v1 archived", "--no-gpg-sign");
git("tag", "v1");
...
check(
  "re-run PRECONDITION_FAILED (no live phases)",
