...
const REPO =
  process.argv[4] ?? "eventually-consistent-code/cairn-drill-scratch";
...
const sh = (cwd, cmd) =>
  execFileSync("bash", ["-c", cmd], { cwd, encoding: "utf8" }).trim();
...
function scaffoldDir(name) {
  ...
  sh(
    dir,
    "git init -q && git config user.email t@t && git config user.name drill",
  );
  writeFileSync(
    join(dir, "cairn.json"),
...
async function runPhase(dir, call, num, name, { failVerify = false } = {}) {
  ...
  decide(
    `phase ${num}: plan has no tasks — create one issue covering the phase goal`,
    "prefer completeness over shortcuts",
  );
  const issue = await call("issue_create", {
    title: `auto p${num}: ${name}`,
    phase: tp?.id,
  });
  await call("plan_issues_set", {
    phaseDir: scaffolded.dir,
    issues: [issue.id],
  });
  ...
  decide(
    `phase ${num}: implement in one file following the repo's existing flat layout`,
    "match existing patterns",
  );
  taste.push(
    `phase ${num}: named the work file work-${num}.txt (naming is taste; reversible)`,
  );
  sh(
    dir,
    `echo done > work-${num}.txt && git add -A && git commit -qm 'feat: p${num}' --no-gpg-sign`,
  );
  ...
  writeFileSync(
    join(dir, ".cairn/plans/phases", scaffolded.dir, "VERIFICATION.md"),
...
console.log("=== leg 1: hands-off 2-phase run ===");
{
  ...
  check(
    "leg1: both phases verified",
    status.phases.every((p) => p.hasVerification),
  );
  const s1 = await call("issue_get", { id: r1.issue.id });
  const s2 = await call("issue_get", { id: r2.issue.id });
  check(
    "leg1: all issues closed",
    s1.state === "closed" && s2.state === "closed",
  );
  check(
    "leg1: every non-taste decision has a principle",
    decisions.length > 0 && decisions.every((d) => d.principle),
  );
  check(
    "leg1: taste batch presented once at end (not mid-run)",
    taste.length >= 2,
  );
  console.log("run report — decisions:");
  for (const d of decisions)
    console.log(`  - ${d.what}  [principle: ${d.principle}]`);
...
console.log("=== leg 2: rigged verify failure → hard stop ===");
{
  ...
  const r1 = await runPhase(dir, call, 1, "auto scree field", {
    failVerify: true,
  });
  check(
    "leg2: hard-stop report names failing task + failure",
    !r1.verified &&
      r1.report.includes(r1.issue.id) &&
      /verify failed/.test(r1.report),
  );
  console.log(`  ${r1.report}`);
  const status = await call("plan_status");
  const p1 = status.phases.find((p) => p.number === 1);
  check(
    "leg2: no VERIFICATION.md written on failed verify",
    p1?.hasVerification === false,
  );
  check(
    "leg2: phase 2 never started (no phase 2 dir, no phase 2 issues)",
    status.phases.length === 1,
  );
  const s1 = await call("issue_get", { id: r1.issue.id });
  check(
    "leg2: failing task left in_progress, not closed",
    s1.state === "in_progress",
  );
  await client.close();
}
...
console.log("=== leg 3: SIGKILL mid-run, fresh session resumes ===");
{
  ...
  const p2 = await call("plan_scaffold_phase", {
    number: 2,
    name: "auto second ascent",
  });
  ...
  const issue2 = await call("issue_create", {
    title: "auto p2: second ascent",
    phase: tp2?.id,
  });
  ...
  check(
    "leg3: handoff survives kill, names phase 2 exact task",
    h.handoff?.issue === issue2.id &&
      h.handoff?.task?.current === "p2-work" &&
      h.stale !== true,
    JSON.stringify(h.handoff ?? h).slice(0, 100),
  );
  const cross = await call("issue_get", { id: issue2.id });
  check(
    "leg3: tracker cross-check matches handoff (in_progress)",
    cross.state === "in_progress",
  );
  const p1After = await call("issue_get", { id: r1.issue.id });
  check(
    "leg3: phase 1 work untouched (still closed, not re-executed)",
    p1After.state === "closed",
  );
  ...
  sh(
    dir,
    "echo done > work-2.txt && git add -A && git commit -qm 'feat: p2 (resumed)' --no-gpg-sign",
  );
  await call("issue_close", { id: issue2.id });
  await call("ledger_append", {
    phaseDir: p2.dir,
    taskRef: "A2",
    summary: "auto drill p2 resumed after SIGKILL",
    baseCommit: base,
    headCommit: sh(dir, "git rev-parse HEAD"),
    issueId: issue2.id,
    closedDate: new Date().toISOString().slice(0, 10),
  });
  writeFileSync(
    join(dir, ".cairn/plans/phases", p2.dir, "VERIFICATION.md"),
  ...
  const status = await call("plan_status");
  check(
    "leg3: run completes from exact task — both phases verified",
