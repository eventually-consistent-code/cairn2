...
const gh = (...a) =>
  execFileSync("gh", ["-R", REPO_SLUG, ...a], { encoding: "utf8" }).trim();
...
check(
  "inbound: first run initializes cursor, reports nothing",
...
const mine = await call("issue_create", {
  title: "cairn-side work item",
  body: "created through the server — write-through should absorb it",
});
const echo = await call("plan_tracker_delta", {});
check(
  "inbound: cairn-side issue_create does not echo as external",
...
const pmUrl = gh(
  ...
  "create",
  "--title",
  "PM added: reporting export",
  "--body",
  "Requested by the PM directly on the tracker.",
);
...
gh(
  ...
  String(mine.id),
  "--title",
  "cairn-side work item (PM retitled)",
);
...
const sawBoth = await eventually(async () => {
  await c.close();
  c = await connect("fidelity-2");
  call = mk(c); // fresh process = fresh 60s read cache
  peek1 = await call("plan_tracker_delta", {});
  return (
    (peek1.new ?? []).some((i) => String(i.id) === pmId) &&
    (peek1.edited ?? []).some((e) => String(e.issue.id) === String(mine.id))
  );
});
const edited = (peek1.edited ?? []).find(
  (e) => String(e.issue.id) === String(mine.id),
);
const titleChange = edited?.changes?.find((ch) => ch.field === "title");
check(
  "inbound: PM-added issue detected as new",
  (peek1.new ?? []).some((i) => String(i.id) === pmId),
  `pm issue #${pmId}`,
);
check(
  "inbound: PM title edit detected with from/to",
  Boolean(titleChange) &&
    titleChange.from === "cairn-side work item" &&
...
check(
  "inbound: un-acked delta re-surfaces on the next peek",
...
const backlogVisible = await eventually(async () => {
  ...
  return arr.some(
    (i) => String(i.id) === pmId && i.labels?.includes("cairn:backlog"),
  );
});
check(
  "inbound: declined item labeled cairn:backlog and visible to status marks",
  backlogVisible,
);
...
check(
  "inbound: ack advances cursor — next peek clean",
...
const closeSeen = await eventually(async () => {
  await c.close();
  c = await connect("fidelity-3");
  call = mk(c);
  const peek3 = await call("plan_tracker_delta", {});
  return (peek3.stateChanged ?? []).some(
    (s) =>
      String(s.issue.id) === pmId && s.from !== "closed" && s.to === "closed",
  );
});
check(
  "inbound: external close detected as stateChanged open→closed",
  closeSeen,
);
await call("plan_tracker_delta", { ack: true });
...
await call("issue_comment", {
  id: String(mine.id),
  text:
    "Starting now — phase 1 base camp, task: cairn-side work item.\n\nbase commit: " +
...
git("add", "-A");
git("commit", "-q", "-m", "feat: the work item lands");
const mins = Math.max(1, Math.round((Date.now() - t0) / 60000));
await call("issue_comment", {
  id: String(mine.id),
  text:
    "Done — the work item shipped with its change committed. Tests green.\n\ncommits: " +
    git("rev-parse", "--short", "HEAD") +
    "\ntime spent: ~" +
    mins +
    "m (approximate)",
...
check(
  "outbound: issue_close accepts timeSpentMinutes on GitHub",
  !closed.__error,
  JSON.stringify(closed.__error ?? "").slice(0, 80),
);
check(
  "outbound: worklogLogged false on non-worklog backend (comment carries time)",
  closed.worklogLogged === false,
);

// verify the paper trail actually exists tracker-side, via gh (independent read)
const comments = JSON.parse(
  gh("issue", "view", String(mine.id), "--json", "comments"),
);
const bodies = comments.comments.map((x) => x.body);
check(
  "outbound: claim comment on the tracker",
  bodies.some((b) => b.startsWith("Starting now")),
);
check(
  "outbound: progress comment on the tracker",
  bodies.some((b) => b.startsWith("Progress")),
);
check(
  "outbound: close comment with time line on the tracker",
  bodies.some(
    (b) => b.includes("time spent: ~") && b.includes("(approximate)"),
  ),
);
...
check(
  "outbound: full lifecycle produced zero external echo",
...
writeFileSync(
  markerPath,
  JSON.stringify({ ...marker, lastScan: "2026-01-01T00:00:00Z" }),
);
const stale = runHook();
writeFileSync(
  markerPath,
  JSON.stringify({ ...marker, lastScan: new Date().toISOString() }),
);
const fresh = runHook();
check(
  "hook: stale cursor → SessionStart nudge present",
  stale.includes("tracker delta unchecked since 2026-01-01"),
);
check(
  "hook: fresh cursor → no nudge",
  !fresh.includes("tracker delta unchecked"),
);
...
c = await connect("fidelity-4");
call = mk(c);
const bad = await call("config_get", {});
check(
  "engineer: mode without handle rejected CONFIG_INVALID",
  bad.__error?.code === "CONFIG_INVALID",
  JSON.stringify(bad.__error ?? "").slice(0, 80),
);
await c.close();

// handle + mode together → loads, mode visible
writeConfig({ user: { handle: "drill-engineer", mode: "engineer" } });
c = await connect("fidelity-5");
call = mk(c);
const good = await call("config_get", {});
check(
  "engineer: handle+mode loads, mode=engineer effective",
...
const hIssue = await call("issue_create", {
  title: "human-claimed: polish the readme",
  body: "Engineer takes this one.",
});
await call("issue_update", {
  id: String(hIssue.id),
  state: "in_progress",
  assignee: "drill-engineer",
});
await call("issue_comment", {
  id: String(hIssue.id),
...
writeFileSync(join(PROJECT, "README.md"), "# fidelity scratch\n\npolished.\n");
git("add", "-A");
git("commit", "-q", "-m", "docs: polish readme (human commit)");
await call("issue_comment", {
  id: String(hIssue.id),
  text: "Done — readme polished and verified. Tests green.\n\ntime spent: ~1m (approximate)\nlogged by cairn for drill-engineer",
});
const hClosed = await call("issue_close", {
  id: String(hIssue.id),
  timeSpentMinutes: 1,
});
const hComments = JSON.parse(
  gh("issue", "view", String(hIssue.id), "--json", "comments,assignees"),
);
check(
  "engineer: human-claimed issue assigned + claim comment on tracker",
  hComments.comments.some((x) =>
    x.body.startsWith("Claimed by drill-engineer"),
  ),
);
check(
  "engineer: close on human's behalf says so ('logged by cairn for')",
  !hClosed.__error &&
    hComments.comments.some((x) =>
      x.body.includes("logged by cairn for drill-engineer"),
    ),
);
...
await c.close();
writeConfig();
c = await connect("fidelity-6");
call = mk(c);
const vibe = await call("config_get", {});
check(
  "engineer: config without user.mode loads clean (vibe default)",
...
console.log("***************************");
console.log(
  `* fidelity drill: ${checks.length - fails}/${checks.length} ${fails ? "FAIL" : "PASS"} *`,
);
console.log("***************************");
process.exit(fails);
