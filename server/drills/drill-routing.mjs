...
const VERBS = resolve(process.argv[4] ?? join(SERVER, "..", "..", "..", "skills", "cairn-trailhead", "verbs"));
...
const FAILURE = "verify FAILED on task 3: expected exit 0 from `npm test`, got 1 — 2 assertions failing in staleness.test.ts";
const hard = await call("trace_start", { description: "drill: verify failure on task 3 (hard route)" });
check("hard route: trace opened for the failure", Boolean(hard.id) && Boolean(hard.issue),
  JSON.stringify(hard.__error ?? "").slice(0, 100));
await call("issue_comment", { id: hard.issue, text: "Investigation started: task 3 verification failed — two test assertions are off. Digging in before touching anything." });
await call("trace_log", { id: hard.id, kind: "evidence", text: FAILURE });

const hardFile = readFileSync(join(PROJECT, ".cairn", "trace", `${hard.id}.md`), "utf8");
const firstEntry = /^## (evidence|hypothesis|test|verdict) — /m.exec(hardFile)?.[1];
check("hard route: failure IS evidence entry #1", firstEntry === "evidence" && hardFile.includes(FAILURE));

// no inline fix: the verdict (with the fix) only lands inside the trace flow
const hardEarly = await call("trace_close", { id: hard.id, resolution: "patched it, probably fine" });
check("hard route: no patch-and-rerun exit — close still gated on a verdict", Boolean(hardEarly.__error));
await call("trace_log", { id: hard.id, kind: "verdict",
  text: "Cause: staleness window widened by DST shift in fixture dates. Fix: fixtures pinned to UTC. Commit: def5678." });
const hardClosed = await call("trace_close", { id: hard.id,
  resolution: "Test fixtures now use UTC dates, so the staleness window no longer shifts with local DST." });
check("hard route: resolved through the trace, issue closed", hardClosed.issueClosed === true);

// -- fast lane: proven-obvious fix in ONE motion ----------------------------
console.log("fast lane: <=3-line fix, one motion, full paper trail...");
const fast = await call("trace_start", { description: "drill: off-by-one in pagination cursor (fast lane)" });
await call("issue_comment", { id: fast.issue, text: "Investigation started: pagination skips the first item on page two — classic off-by-one, fix is obvious." });
await call("trace_log", { id: fast.id, kind: "evidence",
  text: "Page 2 starts at index 21, not 20 — cursor computed as page*size+1." });
await call("trace_log", { id: fast.id, kind: "verdict",
  text: "Cause: cursor off-by-one. Fix: page*size, one line. Commit: 0123abc." });
const fastClosed = await call("trace_close", { id: fast.id,
  resolution: "Pagination cursor fixed — page two now starts exactly where page one ended." });
check("fast lane: one motion start→evidence→verdict→close", fastClosed.issueClosed === true);

const fastList = await call("trace_list", { status: "resolved" });
const fastInfo = fastList.find?.((t) => t.id === fast.id);
check("fast lane: complete session recorded despite single motion",
  fastInfo?.entryCounts?.evidence === 1 && fastInfo?.entryCounts?.verdict === 1);

const fastComments = await readComments(fast.issue);
check("fast lane: both mirror touches (started + resolved close-note)",
  fastComments.length === 2 && /^Investigation started:/.test(fastComments[0])
