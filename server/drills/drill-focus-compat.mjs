...
check(
  "workspace_list: null workspace, not an error",
  ws.workspace === null && !ws.__error,
);
...
check(
  "board_get without a workspace refused with the init hint",
  Boolean(board.__error) &&
    JSON.stringify(board.__error).includes("basecamp init"),
);
...
check(
  "active context under launch-dir .cairn",
  existsSync(join(PROJECT, ".cairn")),
);

const probe = await call("probe_start", {
  description: "compat: does anything move?",
});
check(
  "session file at launch-dir path",
  existsSync(join(PROJECT, ".cairn", "probe", `${probe.id}.md`)),
  JSON.stringify(probe.__error ?? "").slice(0, 80),
);
check(
  "session phase stamped from launch-dir context",
  (await call("session_landscape", {})).sessions.find((s) => s.id === probe.id)
    ?.phase === "2",
);
...
check(
  "audit record at launch-dir path",
  rec.path ===
    join(
      PROJECT,
      ".cairn",
...
await call("map_set", {
  patch: { nodes: { m1: { type: "module", label: "compat" } } },
});
check(
  "map store at launch-dir path",
  existsSync(join(PROJECT, ".cairn", "map", "map.json")),
);
...
await call("probe_log", {
  id: probe.id,
  kind: "verdict",
  text: "VALIDATED — nothing moved",
});
await call("probe_close", {
  id: probe.id,
  resolution: "proceed — compat holds",
});

// no .cairn/basecamp ever appeared
check(
  "no basecamp state materialized",
  !existsSync(join(PROJECT, ".cairn", "basecamp")),
);
