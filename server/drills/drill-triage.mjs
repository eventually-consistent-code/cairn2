...
const unlabeled = await call("issue_create", {
  title: "flaky retry needs a deadline",
  body: "seen twice in CI this week",
});
const bodiless = await call("issue_create", {
  title: "tune the cache TTLs",
  labels: ["cairn:backlog"],
});
const dupA = await call("issue_create", {
  title: "export button unresponsive on large datasets",
  labels: ["cairn:backlog"],
  body: "clicking export with 100k rows does nothing",
});
const dupB = await call("issue_create", {
  title: "export button does nothing on large data sets",
  labels: ["cairn:backlog"],
...
const bug = await call("issue_create", {
  title: "drill: pagination skips first row after sort",
  labels: ["cairn:bug"],
  body: "critical\nSort then paginate loses row one.",
});
const trace = await call("trace_start", {
  description: "drill: pagination skips first row after sort",
  issueId: bug.id,
});
await call("trace_log", {
  id: trace.id,
  kind: "evidence",
  text: "reproduced on any sorted column",
});
await call("trace_log", {
  id: trace.id,
  kind: "verdict",
...
const RESOLUTION =
  "Cursor now resets when sort changes, so page one starts at row one.";
await call("trace_close", { id: trace.id, resolution: RESOLUTION });
await call("issue_update", { id: bug.id, state: "open" });
check(
  "staged: cairn:bug reopened after its trace resolved",
...
check(
  "evidence: landscape carries the trace resolution",
  resolvedTrace?.resolution === RESOLUTION,
);
...
for (const i of open) {
  if (i.labels.includes("cairn:bug") && resolvedTrace && i.id === bug.id) {
    findings.push({
      severity: "important",
      title: "reopened after its trace named a resolution",
      issue: i.id,
    });
    ...
    findings.push({
      severity: "minor",
      title: "possible duplicate of its export-button twin",
      issue: i.id,
    });
  }
}
...
check(
  "record written, findings linked to real issue ids",
  rec1.findings === findings.length && findings.every((f) => f.issue),
  JSON.stringify(rec1.__error ?? "").slice(0, 100),
);
check(
  "report leg staged all five: 1 important + 4 minors",
  findings.filter((f) => f.severity === "important").length === 1 &&
    findings.length === 5,
...
check(
  "report leg mutated nothing (bug still open, unlabeled still bare)",
...
check(
  "label vocabulary read from the project, non-empty",
  vocab.includes("cairn:backlog"),
);
await call("issue_update", { id: unlabeled.id, labels: ["cairn:backlog"] });
check(
  "unlabeled: labeled from existing set only",
  (await call("issue_get", { id: unlabeled.id })).labels.includes(
    "cairn:backlog",
  ),
);
...
check(
  "resolved-but-open: closed with the evidence quoted",
...
await call("issue_comment", {
  id: dupA.id,
  text: `Possible duplicate of ${dupB.id} — same export-button symptom. Flagging for a human call, not closing either.`,
});
await call("issue_comment", {
  id: dupB.id,
  text: `Possible duplicate of ${dupA.id} — same export-button symptom. Flagging for a human call, not closing either.`,
});
const dupsOpen =
  (await call("issue_get", { id: dupA.id })).state === "open" &&
...
check(
  "bodiless: untouched (body still empty, still open)",
...
const rec2 = await call("audit_record", {
  ...
  findings: findings.filter(
    (f) => f.severity === "minor" && f.title !== "unlabeled",
  ),
});
const raw2 = readFileSync(
  join(PROJECT, ".cairn", "audit", `triage-${today}.md`),
  "utf8",
);
check(
  "same-day re-run supersedes the record",
  !raw2.includes("reopened after its trace"),
);
...
for (const id of [unlabeled.id, bodiless.id, dupA.id, dupB.id])
  await call("issue_close", { id });
