...
check("probe_start: session + real cairn:spike issue", Boolean(started.id) && Boolean(started.issue),
  JSON.stringify(started.__error ?? "").slice(0, 100));

const issue = await call(c1, "issue_get", { id: started.issue });
check("spike issue: label cairn:spike, open", issue.labels?.includes("cairn:spike") && issue.state === "open");

const mirror1 = "Investigation started: checking whether the export pipeline can stream a million rows without loading them all into memory. Riskiest assumption first.";
...
writeFileSync(join(artifactDir, "stream-poc.mjs"), "// throwaway: streams fake rows, prints peak RSS\n");

await call(c1, "probe_log", { id: started.id, kind: "experiment",
  text: "stream-poc.mjs pipes 1M synthetic rows through the export transform; validates memory stays flat." });
await call(c1, "probe_log", { id: started.id, kind: "result",
...
await call(c1, "probe_log", { id: started.id, kind: "requirement",
...
check("mid-spike: open, counts 1/1/1/0",
  mid?.status === "open" && mid.entryCounts?.experiment === 1 && mid.entryCounts?.result === 1
  && mid.entryCounts?.requirement === 1 && mid.entryCounts?.verdict === 0);
...
check("resume: counts intact, zero re-derived results",
  resumed?.entryCounts?.experiment === 1 && resumed?.entryCounts?.result === 1
  && resumed?.entryCounts?.requirement === 1);

const sessionFile = join(PROJECT, ".cairn", "probe", `${started.id}.md`);
const lastHeader = [...readFileSync(sessionFile, "utf8")
  .matchAll(/^## (experiment|result|requirement|verdict) — /gm)].at(-1)?.[1];
check("resume: last entry exactly where the kill landed", lastHeader === "requirement");

// -- verdict gate + close proceed -------------------------------------------
const premature = await call(c2, "probe_close", { id: started.id, resolution: "proceed" });
check("verdict gate: close without verdict refused", Boolean(premature.__error));

const mirror2 = "Key finding: streaming holds memory flat at a million rows. One catch — the compression step stalls briefly every hundred thousand rows, so the real build needs progress reporting.";
await call(c2, "issue_comment", { id: started.issue, text: mirror2 });
await call(c2, "probe_log", { id: started.id, kind: "verdict",
  text: "VALIDATED — streaming approach holds; backpressure stalls are a UX issue, not a memory issue." });

const RESOLUTION = "proceed — streaming export is viable; carry the progress-event requirement into the real build.";
const closed = await call(c2, "probe_close", { id: started.id, resolution: RESOLUTION });
check("probe_close: resolved, issue closed", closed.issueClosed === true && closed.gateTexts?.length === 1);
check("archive: live file gone, archive present",
  !existsSync(sessionFile) && existsSync(join(PROJECT, ".cairn", "probe", "archive", `${started.id}.md`)));
check("tracker: issue closed", (await call(c2, "issue_get", { id: started.issue })).state === "closed");

// -- --wrap mechanics: package a real skill with provenance ------------------
console.log("wrap mechanics: packaging the spike into a skill...");
const archive = readFileSync(join(PROJECT, ".cairn", "probe", "archive", `${started.id}.md`), "utf8");
...
writeFileSync(join(skillDir, "SKILL.md"), [
  ...
  "", "## What to avoid", "- Silent backpressure stalls — surface them as progress events.",
  "", "## Origin",
  `- session: ${started.id}`, `- tracker issue: ${started.issue}`,
  `- files: .cairn/probe/${started.id}/stream-poc.mjs`, "",
].join("\n"));
const skill = readFileSync(join(skillDir, "SKILL.md"), "utf8");
check("wrap: package exists with provenance (session id + issue + files)",
  skill.includes(started.id) && skill.includes(started.issue) && skill.includes("stream-poc.mjs"));
...
check("story: three comments in order (started → finding → resolved)",
