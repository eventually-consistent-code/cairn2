...
const git = (...args) =>
  execFileSync("git", ["-C", PROJECT, ...args], { encoding: "utf8" }).trim();
...
console.log(
  "seeding a scratch repo: two ledgered task commits + one overlapping later commit...",
);
git("init", "-q", "-b", "main");
...
mkdirSync(join(PROJECT, ".cairn", "plans", "phases", "01-drill"), {
  recursive: true,
});
...
check(
  "ledger: range appended via ledger_append",
  !led.__error,
...
const ledger = readFileSync(
  join(PROJECT, ".cairn", "plans", "phases", "01-drill", "LEDGER.md"),
  "utf8",
);
const range = /commits ([0-9a-f]{7})\.\.([0-9a-f]{7})/.exec(ledger);
check(
  "revert set: ledger names the exact range",
  range?.[1] === c0.slice(0, 7) && range?.[2] === c2.slice(0, 7),
);
const revertSet = git(
  "rev-list",
  "--reverse",
...
check(
  "revert set: exactly the two task commits",
  revertSet.length === 2 && revertSet[0] === c1 && revertSet[1] === c2,
);

// -- overlap check: later commits touching the same files ------------------------
const manifestFiles = new Set(
  revertSet.flatMap((sha) =>
    git("show", "--name-only", "--format=", sha).split("\n").filter(Boolean),
  ),
);
const laterCommits = git("rev-list", `${range[2]}..HEAD`)
  .split("\n")
  .filter(Boolean);
const overlaps = [];
...
check(
  "overlap: later commit flagged file-by-file",
  overlaps.length === 1 &&
    overlaps[0].sha === later &&
    overlaps[0].file === "b.txt",
);
...
check(
  "apply: revert commit created, a.txt back to v1",
  readFileSync(join(PROJECT, "a.txt"), "utf8") === "v1\n",
);
check(
  "apply: original shas intact — reverts only, no rewrite",
  git("rev-parse", `${headBefore}^{commit}`) === headBefore &&
    git("rev-list", "HEAD").split("\n").includes(c1) &&
    git("log", "-1", "--format=%s").startsWith("Revert"),
);
check(
  "apply: overlapping file untouched by the revert",
...
const rec = await call("audit_record", {
  scope: "backtrack-01-drill",
  verdict: "findings",
  findings: [
    {
      severity: "important",
      title:
        "b.txt overlaps later work — excluded from revert, needs manual review",
    },
  ],
});
