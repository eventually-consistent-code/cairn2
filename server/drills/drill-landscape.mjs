...
const dead = await call("probe_start", {
  description: "drill: third-party OCR for receipts",
});
check(
  "probe under active phase",
  Boolean(dead.id),
  JSON.stringify(dead.__error ?? "").slice(0, 100),
);
await call("probe_log", {
  id: dead.id,
  kind: "experiment",
  text: "ran 50 sample receipts through the OCR trial tier",
});
await call("probe_log", {
  id: dead.id,
  kind: "verdict",
  text: "INVALIDATED — 40% field accuracy, unusable",
});
const STOP =
  "stop — OCR accuracy is nowhere near viable; revisit only if the vendor ships their v3 model.";
await call("probe_close", { id: dead.id, resolution: STOP });

// one open draft for contrast
const open = await call("draft_start", {
  description: "drill: settings page layout",
});
...
check(
  "resolution text carried — the never-re-propose input",
  stopped?.resolution === STOP,
);
check(
  "phase linkage groups the probe under phase 2",
  scape.phases?.some((p) => p.phase === "2" && p.sessions.includes(dead.id)),
);
check(
  "openByKind: 1 draft open, 0 probes",
...
check(
  "two calls byte-equal on unchanged store",
...
await call("draft_log", {
  id: open.id,
  kind: "decision",
  text: "single-column settings, no tabs",
});
