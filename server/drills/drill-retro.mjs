...
const planted = await call("mem_card_create", {
  ...
  scopePhase: 1, confidence: "high",
  provenance: [{ file: "retry.ts", commit: head }],
});
check("planted wrong prior card at high", planted.frontmatter?.confidence === "high");

// -- retro step 1: gather evidence -------------------------------------------
const ledger = readFileSync(join(PROJECT, ".cairn/plans/phases", phase.dir, "LEDGER.md"), "utf8");
const closed = await call("issue_get", { id: issue.id });
check("evidence gathered: ledger line + closed issue",
...
check("lesson card written with provenance + high confidence",
  lesson.frontmatter?.confidence === "high" && lesson.frontmatter?.provenanceFiles?.includes("retry.ts"));

// -- retro step 3: re-grade the contradicted planted card DOWN ----------------
const regraded = await call("mem_card_update", { id: planted.id, confidence: "low" });
check("planted card re-graded high → low via mem_card_update",
...
check("correction written as NEW card (bodies immutable)",
...
check("recall surfaces low confidence on the down-ranked card",
  plantedNow?.frontmatter?.confidence === "low");
const stats = await call("mem_stats");
check("banner re-rendered (stats report banner tokens)", typeof stats.bannerTokens === "number");
