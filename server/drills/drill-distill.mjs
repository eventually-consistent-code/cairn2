...
const CLI = join(dirname(fileURLToPath(import.meta.url)), "..", "..", "hooks", "scripts", "leak-patterns.mjs");

const git = (...args) => execFileSync("git", args, { cwd: PROJECT, encoding: "utf8" }).trim();
const scan = (...files) => spawnSync("node", [CLI, ...files], { cwd: PROJECT, encoding: "utf8" });
...
const phase = await call("plan_scaffold_phase", { number: 1, name: "memory index" });
const ctxPath = join(PROJECT, ".cairn/plans/phases", phase.dir, "CONTEXT.md");
writeFileSync(ctxPath, `# Phase 1: memory index — Context

## Locked decisions

- Use SQLite FTS5 for the memory index — porter stemming, contentless tables.
`);
git("add", "-A"); git("commit", "-m", "chore: scaffold + lock decision", "--no-gpg-sign");
const decisionSha = git("rev-parse", "HEAD");
await call("ledger_append", {
  phaseDir: phase.dir, taskRef: "D1", summary: "FTS5 memory index shipped",
  baseCommit: decisionSha, headCommit: decisionSha, issueId: "DRILL-77",
...
writeFileSync(draftAdr, `# ADR 0001: Use SQLite FTS5 for the memory index
...
check("sanitize gate catches draft internal refs (exit 1)",
  draftScan.status === 1
  && /phase-ref/.test(draftScan.stdout) && /cairn-path/.test(draftScan.stdout) && /tracker-id/.test(draftScan.stdout),
...
check("re-scan clean (exit 0) — the gate", rescan.status === 0, rescan.stdout.trim());
...
const ledgerLine = readFileSync(join(PROJECT, ".cairn/plans/phases", phase.dir, "LEDGER.md"), "utf8")
  .split("\n").find((l) => l.startsWith("- [x]")) ?? "";
const summary = ledgerLine.split(" — ")[1] ?? "work shipped";
writeFileSync(join(PROJECT, "docs", "CHANGELOG.md"),
  `# Changelog\n\n## Milestone 1\n\n- ${summary} (${decisionSha.slice(0, 7)})\n`);
writeFileSync(join(PROJECT, "docs", "ARCHITECTURE.md"),
...
// -- final assertions ----------------------------------------------------------
const finals = ["docs/adr/0001-use-sqlite-fts5.md", "docs/CHANGELOG.md", "docs/ARCHITECTURE.md"]
  .map((f) => join(PROJECT, f));
const finalScan = scan(...finals);
check("zero leak-pattern hits across ALL final docs (mechanically scanned)",
  finalScan.status === 0, finalScan.stdout.trim());
const adrText = readFileSync(finalAdr, "utf8");
check("ADR traces to the locked decision via commit sha",
  adrText.includes(decisionSha.slice(0, 7)) && adrText.includes("SQLite FTS5"));
check("ADR carries no phase dirs, no .cairn, no tracker ids",
  !/phases\/\d{2}|\.cairn\/|DRILL-\d+/.test(adrText));
const changelog = readFileSync(join(PROJECT, "docs", "CHANGELOG.md"), "utf8");
check("CHANGELOG entry derived from the real ledger summary",
