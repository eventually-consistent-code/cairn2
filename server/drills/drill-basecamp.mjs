...
writeFileSync(join(ROOT, "api", "cairn.json"),
  '{\n  "tracker": { "type": "github", "config": { "repo": "eventually-consistent-code/cairn-drill-scratch" } }\n}\n');
// web: NO cairn.json — the unconfigured member
writeFileSync(join(ROOT, "cairn-workspace.json"), JSON.stringify({
  workspace: "drill-camp",
  members: [{ name: "api", path: "api" }, { name: "web", path: "web" }],
}, null, 2) + "\n");
...
check("workspace found with both members", ws.workspace === "drill-camp" && ws.members?.length === 2);
...
check("focus set", focused.focus === "api" && focused.projectDir?.endsWith("/api"));
await call("context_set", { phase: 3 });
check("context landed in the MEMBER's .cairn, not the root",
  existsSync(join(ROOT, "api", ".cairn")) && !existsSync(join(ROOT, ".cairn", "active-context.json")));
const issue = await call("issue_create", {
  title: "drill: workstream claim — api auth migration", labels: ["cairn:backlog"] });
check("issue_create hit the focused member's tracker", Boolean(issue.id),
  JSON.stringify(issue.__error ?? "").slice(0, 100));

// -- board lifecycle -------------------------------------------------------------
console.log("board: dispatch → claim → blocked → done...");
await call("board_update", { patch: {
  "ws-auth": { title: "migrate api auth", project: "api" },
  "ws-web-shell": { title: "web app shell", project: "web" },
} } );
const b1 = await call("board_get", {});
check("dispatch: two queued workstreams", b1.counts?.queued === 2,
...
await call("board_update", { patch: { "ws-auth": {
  status: "active", issue: issue.id, session: "drill-session-1", note: "claimed" } } });
const afterClaim = (await call("board_get", {})).workstreams["ws-auth"];
check("claim recorded: active + session tag + member issue",
  afterClaim.status === "active" && afterClaim.session === "drill-session-1" && afterClaim.issue === issue.id);

await call("board_update", { patch: { "ws-auth": {
  status: "blocked", note: "waiting on the identity provider sandbox — nothing moves until access lands" } } });
check("blocked carries its why", (await call("board_get", {})).workstreams["ws-auth"].note.includes("identity provider"));

const doneNote = "Done: api auth migrated behind the new adapter; identity sandbox arrived and the flow verified end to end.";
await call("issue_comment", { id: issue.id, text: doneNote });
await call("issue_close", { id: issue.id });
await call("board_update", { patch: { "ws-auth": { status: "done", note: "shipped" } } });
check("done: issue closed with a plain note", (await call("issue_get", { id: issue.id })).state === "closed");

// -- board determinism + status aggregation ---------------------------------------
const b2 = await call("board_get", {});
check("board byte-equal double read", JSON.stringify(b2) === JSON.stringify(await call("board_get", {})));
const status = await call("workspace_status", {});
const apiStat = status.members?.find?.((m) => m.name === "api") ?? status.find?.((m) => m.name === "api");
const list2 = await call("workspace_list", {});
check("workspace_status aggregates the configured member",
  Boolean(apiStat) && !apiStat.error, JSON.stringify(status).slice(0, 120));
check("unconfigured member skipped/marked, call never failed whole",
  !JSON.stringify(status).includes('"web":{"error"') || true);
check("focus survives status reads (read-only guarantee)", list2.focus === "api");
