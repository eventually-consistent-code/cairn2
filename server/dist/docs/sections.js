...
const assertBody = (body) => {
    ...
    if (findHeadings(lines).length > 0) {
        throw new CairnError("CONFIG_INVALID", "body must not contain ##+ headings -- that would split the marked region", "write each subsection through its own marked section instead");
    }
    ...
    for (const raw of lines) {
        ...
        if (!inFence && attempt.test(line)) {
            throw new CairnError("CONFIG_INVALID", "body must not contain a docs: marker line", "drop the marker from the body -- the writer manages the marker");
        }
    }
};
...
const findRegion = (lines, heading) => {
    const headings = findHeadings(lines);
    const pos = headings.findIndex((h) => h.heading === heading);
    if (pos === -1) {
        throw new CairnError("NOT_FOUND", `no section heading '${heading}' in the document`, "check the heading text, or create the section explicitly");
    }
    const marker = findMarker(lines, headings[pos].index, DOCS_NAMESPACE);
    if (!marker) {
        throw new CairnError("PRECONDITION_FAILED", `section '${heading}' has no <!-- docs: --> marker -- it is hand-written prose`, "add a '<!-- docs: done -->' line under the heading to opt it into machine updates");
    }
 ...
 */
export function writeSection(markdown, heading, body, opts) {
    assertBody(body);
 ...
 */
export function createSection(markdown, heading, body, opts) {
    assertBody(body);
    const level = opts?.level ?? 2;
    if (!Number.isInteger(level) || level < 2 || level > 6) {
        throw new CairnError("CONFIG_INVALID", `heading level must be an integer 2..6, got '${level}'`, "sections are ##+ headings; pass 2 for '##'");
    }
    if (heading.trim() === "" || heading.includes("\n")) {
        throw new CairnError("CONFIG_INVALID", "heading must be a non-empty single line", "pass the heading text without '#' prefixes or newlines");
    }
    const lines = markdown.split("\n");
 ...
 */
export function writeSectionFile(path, heading, body, opts) {
    let markdown;
    try {
        markdown = readFileSync(path, "utf8");
    }
    catch {
        throw new CairnError("NOT_FOUND", `no document at ${path}`, "pass the path of an existing markdown file");
    }
    let out;
    try {
