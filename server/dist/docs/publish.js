 ...
 */
async function upsert(connector, title, markdown, parentId, context, container, sourceName) {
    ...
    if (!taken)
        return connector.createPage({ title, markdown, parentId, container, sourceName });
...
async function publishNode(connector, node, parentId, context, sink) {
    let page = await upsert(connector, node.title, node.markdown, parentId, context, node.children.length > 0, node.sourceName);
