...
export function appendSession(projectDir, kind, id, entryKind, text) {
    ...
    if (!existsSync(path)) {
        if (existsSync(join(archiveDir(projectDir, kind), `${id}.md`))) {
            throw new CairnError("PRECONDITION_FAILED", `${kind} '${id}' is resolved — archived sessions are immutable`, `start a new ${kind} session if it's back`);
        }
...
export function closeSession(projectDir, kind, id, resolution) {
    ...
    if (gateTexts.length === 0) {
        throw new CairnError("PRECONDITION_FAILED", `${kind} '${id}' has no ${spec.closeGate} entry — close needs one`, `${kind}_log a ${spec.closeGate}, then close`);
    }
