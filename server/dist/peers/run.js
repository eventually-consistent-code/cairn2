 ...
 */
export async function peerRun(projectDir, provider, input, timeoutMs = DEFAULT_TIMEOUT_MS) {
    const cfg = loadConfig(projectDir);
    const peerCfg = cfg.peers?.[provider] ?? {};
    ...
    return new Promise((resolve, reject) => {
        const child = execFile(bin, args, { timeout: timeoutMs, maxBuffer: MAX_BUFFER, killSignal: "SIGKILL" }, (error, stdout, stderr) => {
            ...
            if (error && error.killed) {
                reject(new CairnError("PRECONDITION_FAILED", `peer '${provider}' timed out after ${durationMs}ms`, `raise timeoutMs, or check that the ${provider} CLI responds on stdin`));
                return;
            }
