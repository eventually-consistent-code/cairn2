 ...
 */
export function parseSeatDoc(text, sourcePath) {
    let data;
    let body;
    try {
        ...
        throw new CairnError("CONFIG_INVALID", `seat definition ${sourcePath}: ${detail}`, "seat files open with a flat frontmatter block — see templates/seats/ for the shape");
    }
    ...
    if (!result.success) {
        const parts = result.error.issues.map((i) => {
            ...
            const message = key === "dose" && data.dose === undefined
                ? "required — every seat declares its injection dose (minimal | standard | full)"
                : i.message;
            return `${key}: ${message}`;
        });
        throw new CairnError("CONFIG_INVALID", `seat definition ${sourcePath}: ${parts.join("; ")}`, "fix the named frontmatter field(s) — see templates/seats/ for the shape");
    }
