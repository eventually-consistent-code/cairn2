 ...
 */
export function resolveActivePhase(projectDir) {
...
const text = (uri, body, mimeType) => ({
    contents: [{ uri: uri.href, text: body, mimeType }],
});
/** Reads a plan artifact, or a placeholder when it isn't there yet. */
function readOr(path, placeholder) {
    return existsSync(path) ? readFileSync(path, "utf8") : placeholder;
}
export function registerPlanResources(server, dir) {
    server.registerResource("roadmap", "cairn://plans/roadmap", {
        title: "Project roadmap",
        description: "The project's phase roadmap (.cairn/plans/roadmap.md), read-only.",
        mimeType: "text/markdown",
    }, async (uri) => text(uri, readOr(join(plansRoot(dir()), "roadmap.md"), "no roadmap yet — scaffold one with plan_scaffold_project."), "text/markdown"));
    ...
    const activePhaseFile = (uri, file) => {
        const d = dir();
        const phase = resolveActivePhase(d);
        if (!phase) {
            return text(uri, "no phases scaffolded — create one with plan_scaffold_phase.", "text/markdown");
        }
        return text(uri, readOr(join(plansRoot(d), "phases", phase.dir, file), `no ${file} for phase ${phase.number} (${phase.dir}) yet.`), "text/markdown");
    };
    server.registerResource("active-phase-plan", "cairn://plans/active-phase/plan", {
        title: "Active phase plan",
        description: "PLAN.md for the active phase (active context when set, else the " +
            "lowest unverified phase), read-only.",
        mimeType: "text/markdown",
    }, async (uri) => activePhaseFile(uri, "PLAN.md"));
    server.registerResource("active-phase-context", "cairn://plans/active-phase/context", {
        title: "Active phase context",
        description: "CONTEXT.md for the active phase (active context when set, else the " +
            "lowest unverified phase), read-only.",
        mimeType: "text/markdown",
    }, async (uri) => activePhaseFile(uri, "CONTEXT.md"));
    server.registerResource("outlook-snapshot", "cairn://outlook/snapshot", {
        title: "Outlook snapshot",
        description: "The project's outlook mirror snapshot (phases, sessions, tracker " +
            "rollup) as JSON, read-only.",
        mimeType: "application/json",
    }, async (uri) => {
        const snapshot = readOutlook(dir());
        if (!snapshot) {
            return text(uri, "no snapshot yet — any write-through tool call emits one.", "text/plain");
        }
