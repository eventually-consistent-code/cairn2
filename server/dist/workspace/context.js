...
const focusPath = (root) => join(root, ".cairn", "basecamp", "focus.json");
...
function readFocus(root) {
    ...
    try {
        ...
        throw new CairnError("CONFIG_INVALID", `${path} is not valid JSON: ${e}`, "inspect or delete .cairn/basecamp/focus.json");
    }
    const focus = parsed?.focus;
    return typeof focus === "string" ? focus : null;
}
function writeFocus(root, focus) {
    const path = focusPath(root);
 ...
 */
export function findWorkspace(launchDir) {
    let dir = resolve(launchDir);
    for (;;) {
        const path = join(dir, WORKSPACE_FILE);
        if (existsSync(path))
            return loadWorkspace(path, dir);
...
function loadWorkspace(path, root) {
    let raw;
    try {
        raw = readFileSync(path, "utf8");
    }
    catch (e) {
        throw new CairnError("CONFIG_INVALID", `${path} could not be read: ${e}`, `fix or remove ${WORKSPACE_FILE}`);
    }
    let parsed;
    try {
        parsed = JSON.parse(raw);
        ...
        throw new CairnError("CONFIG_INVALID", `${path} is not valid JSON: ${e}`, `fix or remove ${WORKSPACE_FILE}`);
    }
    if (typeof parsed !== "object" || parsed === null) {
        throw new CairnError("CONFIG_INVALID", `${path} must contain a JSON object`, `fix ${WORKSPACE_FILE} — expected { workspace, members }`);
    }
    const obj = parsed;
    if (typeof obj.workspace !== "string" || obj.workspace.trim().length === 0 || !Array.isArray(obj.members)) {
        throw new CairnError("CONFIG_INVALID", `${path} is missing a 'workspace' name or a 'members' array`, `fix ${WORKSPACE_FILE} — expected { workspace: string, members: [{ name, path }] }`);
    }
    const members = obj.members.map((raw, i) => {
        if (typeof raw !== "object" || raw === null
            || typeof raw.name !== "string"
            || typeof raw.path !== "string") {
            throw new CairnError("CONFIG_INVALID", `${path}: member ${i} is missing 'name' or 'path'`, `fix ${WORKSPACE_FILE} — every member needs a name and a path`);
        }
        const m = raw;
 ...
 */
export function setFocus(launchDir, project) {
    ...
    if (!member) {
        throw new CairnError("CONFIG_INVALID", `'${project}' is not a member of workspace '${info.workspace}'`, `known members: ${info.members.map((m) => m.name).join(", ") || "(none)"}`);
    }
    if (!member.configured) {
        throw new CairnError("CONFIG_INVALID", `member '${project}' has no cairn.json and cannot be focused`, `add a cairn.json to ${member.path}`);
    }
 ...
 */
export function resolveProjectDir(launchDir) {
    const info = findWorkspace(launchDir);
    if (!info || info.focus === null)
        return launchDir;
    const member = info.members.find((m) => m.name === info.focus);
    if (!member || !member.configured) {
        throw new CairnError("CONFIG_INVALID", `focus names '${info.focus}' but it is ${member ? "no longer configured (missing cairn.json)"
            : "no longer a member"} of workspace '${info.workspace}'`, `clear focus (workspace_focus with project: null) or restore '${info.focus}' as a configured member`);
    }
