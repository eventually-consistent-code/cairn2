 ...
 */
export function finalizeMigration(storeDir, targetType, result) {
...
export async function migrateTracker(src, dst) {
    const remap = {};
    const phaseRemap = {};
    ...
    for (const i of issues) {
        await attempt(`issue '${i.id}'`, async () => {
            ...
            counts.issues++;
            if (i.category !== "open")
                await dst.updateIssue(made.id, { state: i.category });
            if (i.assignee) {
                await attempt(`assignee on '${i.id}'`, () => dst.updateIssue(made.id, { assignee: i.assignee }).then(() => undefined));
            }
        });
    }
    ...
    for (const i of issues) {
        ...
        for (const c of await src.listComments?.(i.id) ?? []) {
            ...
            if (await attempt(`comment on '${i.id}'`, () => dst.commentIssue(newId, `${prefix}${c.text}`).then(() => undefined))) {
                counts.comments++;
            }
        }
        for (const w of await src.listWorklogs?.(i.id) ?? []) {
            const carried = dst.capabilities.hasWorklog && dst.logWork
                ...
                : () => dst.commentIssue(newId, `[worklog ${w.at ?? "?"} ${w.author ?? "?"}] ${w.minutes}m`).then(() => undefined);
            if (await attempt(`worklog on '${i.id}'`, carried))
                counts.worklogs++;
        }
    }
    ...
    for (const l of await src.listLinks?.() ?? []) {
        ...
        if (dst.linkIssues) {
            if (await attempt(`link ${l.from} ${l.type} ${l.to}`, () => dst.linkIssues(from, l.type, to)))
                counts.links++;
        }
        else {
            warnings.push(`link ${l.from} ${l.type} ${l.to}: target has no link support — recorded as a comment`);
            if (await attempt(`link comment on '${l.from}'`, () => dst.commentIssue(from, `[link] ${l.type} → ${to}`).then(() => undefined))) {
                counts.links++;
            }
        }
    }
