...
async function fetchRaw(fetchImpl, url, init, opts = {}) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        if (resp.status === 401 || resp.status === 403) {
            ...
            throw new CairnError("AUTH_MISSING", tag(`HTTP ${resp.status} from ${url}`), "check the token env var for this backend");
        }
        if (resp.status === 404)
            throw new CairnError("NOT_FOUND", tag(`404 from ${url}`));
        if (resp.status === 429 || resp.status >= 500) {
            lastErr = new CairnError(resp.status === 429 ? "RATE_LIMITED" : "TRACKER_DOWN", tag(`HTTP ${resp.status} from ${url}`));
            if (attempt < retries)
                await sleep(backoffMs * 2 ** attempt);
            continue;
        }
