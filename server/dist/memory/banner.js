 ...
 */
function scopedCards(cards, active) {
    const issueCards = [];
    const phaseCards = [];
    const projectCards = [];
    for (const card of cards) {
        ...
        if (scopeIssue !== undefined) {
            if (active.issueId !== undefined && scopeIssue === active.issueId)
                issueCards.push(card);
        }
        else if (scopePhase !== undefined) {
            if (active.phase !== undefined && scopePhase === String(active.phase))
...
function computeBannerData(projectDir) {
    ...
    const rows = cards.map((card) => `| ${card.id} | ${card.frontmatter.type} | ${titleFor(card.body)} | ~${fetchCost(card.body)} tok |`);
 ...
 */
export function writeBanner(projectDir) {
    try {
 ...
 */
export function bannerStats(projectDir) {
    try {
