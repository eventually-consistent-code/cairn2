...
const MARKER_RE = new RegExp("^<!--\\s*([a-z]+):\\s+(done|pending|failed)"
...
const buildMarker = (namespace, state, meta) => {
    ...
    if (meta?.model !== undefined && !MODEL_RE.test(meta.model)) {
        throw new CairnError("CONFIG_INVALID", `model must be a lowercase tier token ([a-z0-9-]+), got '${meta.model}'`, "use a token like sonnet, haiku, or opus-4-6, or omit it");
    }
    if (meta?.note !== undefined && (meta.note.includes("-->") || meta.note.includes("\n"))) {
        throw new CairnError("CONFIG_INVALID", "note must be a single line and cannot contain '-->'", "reword the note without a comment terminator or newline");
    }
...
const findHeadings = (lines) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (m)
            out.push({ heading: m[2], level: m[1].length, index: i });
    }
    return out;
};
...
const findMarker = (lines, headingIndex, namespace) => {
    ...
    for (let i = headingIndex + 1; i < lines.length; i++) {
        ...
        if (!COMMENT_RE.test(line))
            break; // first real content ends the block
        if (!attempt.test(line))
            continue; // other namespace (or plain comment): ignored
        const match = MARKER_RE.exec(line);
        if (!match || match[1] !== namespace) {
            throw new CairnError("CONFIG_INVALID", `line ${i + 1}: '${line}' looks like a ${namespace} section marker but does not parse`, `fix the marker to '<!-- ${namespace}: done|pending|failed [YYYY-MM-DD] [model] [— note] -->'`);
        }
 ...
 */
export function flipSection(markdown, namespace, heading, state, meta) {
    assertNamespace(namespace);
    const marker = buildMarker(namespace, state, meta);
    const lines = markdown.split("\n");
    const target = findHeadings(lines).find((h) => h.heading === heading);
    if (!target) {
        throw new CairnError("NOT_FOUND", `no section heading '${heading}' in the document`, "check the heading text with a parse-only call first");
    }
